...
LETTERS = ascii_uppercase + "ÇÁÉÍÓÚÀÂÊÔÃÕ"
...
class Hangman:
    ...
    def _restart_game(self):
        ...
        self._window["-MOSTRAR-PALAVRA-"].update(self._guessed_word)        
    ...
    def check_winner(self):
        ...
        if self._wrong_guesses < MAX_WRONG_GUESSES:
            ...
            answer = sg.PopupYesNo(
                ...
                f"Você acertou {self._won_games} de {self._played_games}!\n"
                "Deseja jogar outro round?",
                title="Vencedor!",
            )
        else:
            answer = sg.PopupYesNo(
                f"Você perdeu! A palavra era '{self._target_word}'.\n"
                f"Você acertou {self._won_games} de {self._played_games}!\n"
                "Deseja jogar outro round?",
                title="Sinto muito!",
            )
        self.quit = answer == "Não"
        if not self.quit:
            self._new_game()                   
...
if __name__ == "__main__":
    ...
    while not game.quit:
        ...
        while not game.is_over():
            ...
            if event_id in {sg.WIN_CLOSED, "-SAIR-"}:
                game.quit = True
                break
